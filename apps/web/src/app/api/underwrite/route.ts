...
export async function POST(request: Request) {
  try {
    ...
    if (!body?.smb || !body?.buyer || !body?.faceAmount || !body?.dueDateSec || !body?.docRef) {
    ...
    if (!process.env.RPC_URL) return NextResponse.json({ error: "server not configured (RPC_URL)" }, { status: 500 });
